    ...
    def models_from_features(cls, features: list[QgsFeature]) -> list[Plan]:
        ...
        return [
            Plan(
                ...
                id_=feature["id"],
                plan_matter_id=feature["plan_matter_id"],
                approval_date=feature["approval_date"] or None,
...
class PlanObjectLayer(PlanObjectReader, AbstractPlanLayer):
    @classmethod
...
class PointLayer(PlanObjectLayer):
    name = "Pisteet"
    association_attribute = "point_id"
    filter_template = Template("plan_id = '$plan_id'")


class LineLayer(PlanObjectLayer):
    name = "Viivat"
    association_attribute = "line_id"
    filter_template = Template("plan_id = '$plan_id'")


class LandUseAreaLayer(PlanObjectLayer):
    name = "Aluevaraus"
    association_attribute = "land_use_area_id"
    filter_template = Template("plan_id = '$plan_id'")


class OtherAreaLayer(PlanObjectLayer):
    name = "Osa-alue"
    association_attribute = "other_area_id"
    filter_template = Template("plan_id = '$plan_id'")


class RegulationGroupLayer(RegulationGroupReader, AbstractPlanLayer):
...
class PlanRegulationLayer(RegulationReader, AbstractPlanLayer):
    name = "Kaavamääräys"
    filter_template = Template(
        dedent(
            """\
...
class AdditionalInformationLayer(AdditionalInformationReader, AbstractPlanLayer):
...
# Bound here because the members reference each other. Never import valid_layers from this module.
AbstractFeatureLayer.family = LayerFamily(
    regulation_group_association=RegulationGroupAssociationLayer,
    regulation_group=RegulationGroupLayer,
    plan_regulation=PlanRegulationLayer,
    plan_proposition=PlanPropositionLayer,
    additional_information=AdditionalInformationLayer,
    plan_theme_association=PlanThemeAssociationLayer,
    type_of_verbal_regulation_association=TypeOfVerbalRegulationAssociationLayer,
)
