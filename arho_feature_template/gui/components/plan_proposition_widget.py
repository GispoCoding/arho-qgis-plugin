...
class PropositionWidget(QWidget, FormClass):  # type: ignore
    ...
    def __init__(self, proposition: Proposition, parent=None):
        ...
        add_field_menu.addAction(tr("Suositusnumero")).triggered.connect(self._add_proposition_number)
        add_field_menu.addAction(tr("Kaavoitusteema")).triggered.connect(self._add_theme)
        ...
        self._add_widget(RequiredFieldLabel(tr("Sisältö:")), self.text_input)
    ...
    def _add_proposition_number(self, default_value: int | None = None):
        if not self.proposition_number_widget:
            ...
            self._add_widget(QLabel(tr("Suositusnumero")), self.proposition_number_widget)
