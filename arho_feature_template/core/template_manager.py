...
class TemplateManager:
    ...
    @classmethod
    def _write_to_yaml_file(cls, config_data: dict, file_path: Path, overwrite: bool) -> bool:  # noqa: FBT001
        ...
        existing_data = cls._read_from_yaml_file(file_path=file_path, fail_msg="")
        existing_data.update(cleaned_data)

        with file_path.open("w", encoding="utf-8") as yaml_file:
            yaml.safe_dump(existing_data, yaml_file, sort_keys=False, allow_unicode=True, default_flow_style=False)
        return True
    ...
    def save_plan_object_to_library(cls, plan_object_model: PlanObject, library: PlanFeatureLibrary):
        ...
        plan_object_saved = TemplateManager.write_plan_feature_template_file(
            plan_feature_config_data=library.into_template_dict(), file_path=Path(library.file_path), overwrite=False
        )
        if plan_object_saved:
            iface.messageBar().pushSuccess("", "Kaavakohde tallennettu kaavakohdepohjakirjastoon.")
        else:
            iface.messageBar().pushCritical("", "Kaavakohteen tallentaminen kaavakohdekirjastoon epäonnistui.")
            library.plan_features.remove(plan_object_model)
            return
        ...
        SIGNAL_MANAGER.object_added_to_library.emit()


class SignalManager(QObject):
    object_added_to_library = pyqtSignal()
